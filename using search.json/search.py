...
def worker(key, value):
    ...
    if ol_response_data.get('numFound') == 0:
        try:
            ...
            print(f"Error occurred while making a request: {e}")
            with open("using search.json/error.jsonl", "a") as error_file:
                error_file.write(json.dumps({key: str(e)}) + "\n")
            return
        ...
        if ol_response_data.get('numFound') == 0:
            ...
            try:
                with open("using search.json/Not_Found.jsonl", "a") as not_found_file:
            ...
            try:
                with open("using search.json/Hits.jsonl", "a") as details_file:
